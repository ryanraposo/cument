...
assert status['project'] == '/home/ryan/repos' and status['source'] == 'terminal', state()
cli('context', '/tmp', '--source', 'nautilus')
status = json.loads(state()['status'])
assert status['project'] == '/tmp' and status['source'] == 'nautilus', state()
probe('Focus', '(i)', (10001,))
settle()
status = json.loads(state()['status'])
assert status['project'] == '/home/ryan/repos' and status['source'] == 'terminal', state()
...
cli('open', root)
settle()
print('WAYLAND PASS: exact geometry, synced earmark/drawer toggle, latest terminal-or-Nautilus context, focus routing, focus restoration')
